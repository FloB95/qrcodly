...
Forward your QR code scan data to your own analytics tools. QRcodly supports **Google Analytics 4** (via the Measurement Protocol) and **Matomo** (via the Tracking API), so you can analyze scan activity alongside all your other traffic data.
...
- **Google Analytics 4** — uses the GA4 Measurement Protocol to send server-side events
- **Matomo** — uses the Matomo Tracking API to record visits

You can connect one or both providers at the same time. Each scan event is forwarded independently.
...
<Callout type="info">
	...
	is zeroed, and IPv6 addresses are truncated. This ensures compliance with GDPR and other privacy
	regulations.
</Callout>
...
QRcodly validates your credentials immediately. If the Measurement ID format is invalid, you'll see an error right away. Once saved, scan events will start flowing to your GA4 property.
