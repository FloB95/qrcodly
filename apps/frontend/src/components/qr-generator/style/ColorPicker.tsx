...
export function ColorPicker({
	...
	const [gradientColors, setGradientColors] = useState<[string, string]>(['#accbee', '#e7f0fd']);
	...
	const backgroundToStyle = useCallback((background: BackgroundType): string => {
		if (typeof background === 'string') return background;
		const gradientType = background.type === 'radial' ? 'radial-gradient' : 'linear-gradient';
		const rotationStr = background.type === 'linear' ? `${background.rotation}deg, ` : '';
	...
	const [debouncedInternalBackground] = useDebouncedValue(internalBackground, 100);
	...
	const handleRotationChange = useCallback(
		(newRotation: number) => {
			...
			if (typeof internalBackground === 'object' && internalBackground.type === 'linear') {
				setInternalBackground({
					...internalBackground,
				...
				>
					<div className="flex w-full items-center gap-2">
						...
						<div className="flex-1 truncate">
							{internalBackground ? backgroundToButtonText(internalBackground) : 'Pick a color'}
						</div>
					</div>
				</Button>
			</PopoverTrigger>
					...
					<TabsContent value="solid" className="mt-0 flex flex-wrap gap-1">
						...
						<div className="w-full">
							<Input
								type="color"
								value={
									typeof internalBackground === 'string' ? internalBackground : gradientColors[0]
							...
							<div className="mb-2 flex flex-wrap gap-1">
								{gradients.map((s, i) => (
									<div
										...
										className="h-6 w-6 cursor-pointer rounded-md active:scale-105"
										onClick={() => handleGradientPresetSelect(s as [string, string])}
									/>
								))}
							</div>
